...
function AdminAnalyticsPage() {
  ...
  if (analyticsQuery.isLoading || statsQuery.isLoading) {
    return <LoadingSpinner label="Loading analytics charts" />
  }
    ...
    <div className="grid gap-6">
      <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <p className="mt-3 max-w-2xl leading-7 text-slate-600">
          Revenue, order volume, user growth, and category performance from MongoDB aggregation.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {statCards.map(({ label, value, icon: Icon, style }) => (
          <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm" key={label}>
            <div className={`mb-4 grid size-11 place-items-center rounded-2xl ${style}`}>
              <Icon size={22} />
            </div>
